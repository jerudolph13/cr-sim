...
## Classical g-computation
  # Time-ordering: LLast, XLast, L, X, Z
    ...
    mod.Z <- multinom(Z ~ X + L + LLast + XLast + as.factor(Int), data=boot)
...
# Run outcome models
    ...
    # Cumulative distribution function for Outcome 1, with  Outcome 2 removed
...
all.res <- mclapply(1:nsim, function(x) {simloop(x)}, mc.cores=cores, mc.set.seed=FALSE)
  # Use lapply to test the code if there are errors
  #all.res <- lapply(1:nsim, function(x) {simloop(x)})
all.res <- do.call(rbind, all.res)

# Summarize over simulations
write.table(all.res, file="../results/cr-sim_allest.txt", sep="\t", row.names=FALSE)
