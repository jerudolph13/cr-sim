...
simulation <- function (exposure, K) {
  ...
  if (K==2) {
    ...
    gamma.vec <- rep(log(lambda/2))
  }
  muK <- sum(exp(gamma.vec))
  
  # Prepare empty vectors to store data
  ...
  cval <- 30
    ...
    for (i in 1:n) {
      ...
      if (K==2) {
        ...
        T0 <- rexp(1, lambda/2) # With 1 event, rate is halved
      }
...
gen_truth <- function (exp, events) {
  set.seed(123)
  sim.dat <- simulation(exposure=exp, K=events) %>% 
    mutate(last = as.numeric(!duplicated(ID, fromLast=T))) %>% 
    filter(last==1) %>% 
    select(-last)
  
  if (events==1) {
    fit <- survfit(Surv(Tv, Z) ~ 1, data=sim.dat)
    sfit <- data.frame(time = fit$time, surv = fit$surv)
  } else {
    sfit <- cuminc(ftime=sim.dat$Tv, fstatus=sim.dat$Z)
  }
...
# Exposure = 1
sfit <- gen_truth(exp=1, events=1)
  truth$r1[3] <- 1 - min(sfit$surv)

# Exposure = 0  
sfit <- gen_truth(exp=0, events=1)
...
# Output results
write.table(truth, file="../results/crsim_truth.txt", sep="\t", row.names=FALSE)
